...
```bash
docker compose up mongodb -d  # or: podman compose up mongodb -d
```
...
We provide Docker Compose configurations for both development and production environments, along with multi-stage Dockerfiles for optimized production images. The production setup includes an Nginx reverse proxy for TLS termination and routing, and the option to use a hardened Dockerfile with `dumb-init`, health checks, and resource limits. Podman users can use the same compose files with `podman compose`.

Alternatively, you can deploy to any cloud provider or Kubernetes cluster using the provided manifests and Terraform modules. 
